...
// 实体管理集成测试
async function testEntityIntegration() {
  console.log('🔄 开始实体管理集成测试...\n');
  ...
  const tests = [
    {
      ...
      test: async () => {
        const response = await axios.post(`${BASE_URL}/projects`, {
          name: '实体集成测试项目',
          code: 'entity_integration_test',
          description: '用于实体集成测试的项目',
          version: '1.0.0'
        });
    ...
    },
    {
      ...
      test: async () => {
        ...
        return {
          success: response.status === 200 && response.data.length >= 2,
          data: { count: response.data.length },
          message: '实体列表包含多个实体'
        };
      }
...
async function main() {
  ...
  const results = await testEntityIntegration();
...
module.exports = { testEntityIntegration };
