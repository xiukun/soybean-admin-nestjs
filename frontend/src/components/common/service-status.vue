        ...
        <div class="service-details">
            ...
            <div v-for="(config, key) in serviceConfigs" :key="key" class="service-item">
              <NSpace align="center" justify="space-between">
                <NSpace align="center">
                  <NIcon 
                    size="12" 
                    :color="serviceHealth[key] ? '#18a058' : '#d03050'"
                  >
                    <icon-mdi:circle />
                  </NIcon>
                ...
                <NTag 
                  size="small" 
                  :type="serviceHealth[key] ? 'success' : 'error'"
                >
          ...
          <NSpace justify="space-between" align="center">
            <NText depth="3" class="text-xs">
              {{ $t('common.lastCheck') }}: {{ lastCheckTime }}
            </NText>
            
            <NButton size="tiny" @click="checkHealth" :loading="checking">
              {{ $t('common.refresh') }}
            </NButton>
          </NSpace>
        </div>
...
const serviceConfigs = getAllServices().reduce((acc, config, index) => {
