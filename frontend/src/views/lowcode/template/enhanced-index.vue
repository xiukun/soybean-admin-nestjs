...
const columns: DataTableColumns<Template> = [
  ...
  { title: $t('page.lowcode.template.language'), key: 'language', width: 120 },
  {
    ...
    key: 'status',
    width: 100,
    render: row => h('NTag', { type: getStatusType(row.status) }, $t(`page.lowcode.template.status.${row.status}`))
  },
...
function getTemplateActions(template: Template) {
  return [
    ...
    {
      label:
        template.status === 'PUBLISHED' ? $t('page.lowcode.template.unpublish') : $t('page.lowcode.template.publish'),
      key: 'publish',
...
<template>
  <div class="enhanced-template-management">
            ...
            <NCard class="mb-4">
              <NSpace justify="space-between" align="center">
                ...
                <NSpace>
                  ...
                  <NButton :disabled="selectedTemplates.length === 0" @click="handleExportTemplates">
                    <template #icon>
    ...
    <NModal v-model:show="showEditorModal" preset="card" style="width: 95vw; height: 90vh">
      ...
      <TemplateEditor :template-data="editingTemplate" @save="handleSaveTemplate" @cancel="showEditorModal = false" />
    </NModal>
    ...
    <NModal v-model:show="showPreviewModal" preset="card" style="width: 80vw; height: 80vh">
      ...
      <TemplatePreview v-if="previewTemplate" :template-data="previewTemplate" />
    </NModal>
