...
import { NButton, NButtonGroup, NIcon, NPopconfirm, NSpace, NTag, useMessage } from 'naive-ui';
import { fetchDeleteRelationship, fetchGetProjectList, fetchGetRelationshipList } from '@/service/api';
  ...
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <NCard v-if="currentProjectId" :bordered="false" size="small">
      <NSpace align="center" justify="space-between">
        <NSpace>
          <NButtonGroup>
            <NButton :type="currentView === 'list' ? 'primary' : 'default'" @click="currentView = 'list'">
              <template #icon>
                <NIcon><icon-mdi-view-list /></NIcon>
              </template>
              {{ $t('page.lowcode.relationship.listView') }}
            </NButton>
            <NButton :type="currentView === 'designer' ? 'primary' : 'default'" @click="currentView = 'designer'">
              <template #icon>
    ...
    <!-- 列表视图 -->
    <div v-if="currentProjectId && currentView === 'list'" class="flex flex-col flex-1 gap-4">
      <RelationshipSearch v-model:model="searchParams" @reset="resetSearchParams" @search="getTableData" />
      <NCard
        :title="$t('page.lowcode.relationship.title')"
        :bordered="false"
        size="small"
    ...
    <!-- 可视化设计器视图 -->
    <div v-if="currentProjectId && currentView === 'designer'" class="h-full min-h-600px flex-1">
      <RelationshipDesigner
        :project-id="currentProjectId"
        @relationship-updated="getTableData"
