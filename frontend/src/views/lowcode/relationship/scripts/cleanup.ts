...
const CLEANUP_CONFIG: CleanupConfig = {
  ...
  directoriesToClean: ['src/views/lowcode/relationship/temp', 'src/views/lowcode/relationship/backup'],
        ...
        if (imports) {
          for (const importStatement of imports) {
            ...
            if (usedItems.length !== importedItems.length && usedItems.length > 0) {
              const newImportStatement = importStatement.replace(/{([^}]+)}/, `{ ${usedItems.join(', ')} }`);
