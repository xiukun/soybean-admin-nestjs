...
// 计算属性
const hasTestResult = computed(() => Boolean(testResult.value));
...
function handleApiSelect(apiId: string) {
  ...
  if (selectedApiConfig.value) {
    testConfig.value.headers = [{ key: 'Content-Type', value: 'application/json' }];
  ...
  <div class="api-config-online-test">
    <NCard :title="$t('page.lowcode.apiConfig.onlineTest.title')" :bordered="false" size="small">
      <template #header-extra>
        <NSpace>
          <NButton :loading="historyLoading" @click="loadTestHistory">
            <template #icon>
      ...
      <NSpace vertical :size="16">
        <!-- API选择 -->
        <div>
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.selectApi') }}</h3>
          <NSpace>
            <NSelect
        ...
        <div v-if="selectedApiConfig">
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.apiInfo') }}</h3>
          <NCard size="small">
        ...
        <!-- 测试配置 -->
        <div v-if="selectedApiConfig">
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.testConfig') }}</h3>
          <NCard size="small">
            <NSpace vertical :size="12">
              <!-- 请求头 -->
              <div>
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.headers') }}</h4>
                <div v-for="(header, index) in testConfig.headers" :key="index" class="mb-2 flex gap-2">
                  <NInput
              ...
              <!-- 环境变量 -->
              <div>
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.envVariables') }}</h4>
                <div v-for="(variable, index) in testConfig.envVariables" :key="index" class="mb-2 flex gap-2">
                  <NInput
                ...
                <NButton size="small" @click="addEnvVariable">
                  <template #icon>
                    <SvgIcon icon="ic:round-add" />
                  </template>
              ...
              <div v-if="selectedApiConfig.method === 'GET'">
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.queryParams') }}</h4>
                <div v-for="(param, index) in testConfig.queryParams" :key="index" class="mb-2 flex gap-2">
                  <NInput
                  ...
                  <NInput
                    ...
                    style="flex: 1"
                  />
                  <NButton type="error" size="small" @click="removeQueryParam(index)">
                    <template #icon>
                      <SvgIcon icon="ic:round-remove" />
                    </template>
                  </NButton>
                </div>
                <NButton size="small" @click="addQueryParam">
                  <template #icon>
                    <SvgIcon icon="ic:round-add" />
                  </template>
              ...
              <div v-if="['POST', 'PUT', 'PATCH'].includes(selectedApiConfig.method)">
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.requestBody') }}</h4>
                <NSpace vertical :size="8">
                  ...
                  <div v-else-if="testConfig.bodyType === 'form'">
                    <div v-for="(field, index) in testConfig.formData" :key="index" class="mb-2 flex gap-2">
                      <NInput
                      ...
                      <NInput
                        ...
                        style="flex: 1"
                      />
                      <NButton type="error" size="small" @click="removeFormField(index)">
                        <template #icon>
                          <SvgIcon icon="ic:round-remove" />
                        </template>
                      </NButton>
                    </div>
                    <NButton size="small" @click="addFormField">
                      <template #icon>
                        <SvgIcon icon="ic:round-add" />
                      </template>
        ...
        <div v-if="selectedApiConfig">
          <NSpace>
            <NButton type="primary" :loading="testLoading" size="large" @click="executeTest">
              <template #icon>
                <SvgIcon icon="ic:round-play-arrow" />
              </template>
              {{ $t('page.lowcode.apiConfig.onlineTest.execute') }}
            </NButton>
            <NButton :disabled="!hasTestResult" @click="saveTestCase">
              <template #icon>
        ...
        <div v-if="showTestCases">
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.testCases') }}</h3>
          <NCard size="small">
            <NSpace vertical :size="12">
              <div class="flex items-center justify-between">
                <span class="font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.savedCases') }}</span>
                <NButton size="small" @click="loadTestCases">
                  <template #icon>
              ...
              <div v-if="testCases.length === 0" class="py-4 text-center text-gray-500">
                {{ $t('page.lowcode.apiConfig.onlineTest.noCases') }}
              </div>

              <div v-else>
                <div v-for="(testCase, index) in testCases" :key="index" class="mb-2 border rounded p-3">
                  <div class="mb-2 flex items-start justify-between">
                    ...
                    <NSpace>
                      <NButton size="small" @click="loadTestCase(testCase)">
                        <template #icon>
                          <SvgIcon icon="ic:round-play-arrow" />
                        </template>
                        {{ $t('page.lowcode.apiConfig.onlineTest.load') }}
                      </NButton>
                      <NButton type="error" size="small" @click="deleteTestCase(index)">
                        <template #icon>
        ...
        <!-- 测试结果 -->
        <div v-if="testResult">
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.result') }}</h3>
          <NCard size="small">
            <NSpace vertical :size="12">
              <!-- 响应状态 -->
              ...
              <!-- 响应头 -->
              <div>
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.responseHeaders') }}</h4>
              ...
              <!-- 响应体 -->
              <div>
                <h4 class="mb-2 font-medium">{{ $t('page.lowcode.apiConfig.onlineTest.responseBody') }}</h4>
                <NTabs type="line">
        ...
        <div v-if="showHistory && testHistory.length > 0">
          <h3 class="mb-3 text-lg font-semibold">{{ $t('page.lowcode.apiConfig.onlineTest.testHistory') }}</h3>
          <NCard size="small">
            <NList>
