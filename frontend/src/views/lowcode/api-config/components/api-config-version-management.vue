  ...
  <div class="api-config-version-management">
    <NCard :title="$t('page.lowcode.apiConfig.versionManagement.title')" :bordered="false" size="small">
      <template #header-extra>
        <NSpace>
          <NButton type="primary" @click="createVersion" :disabled="!selectedApiId">
            <template #icon>
              <SvgIcon icon="ic:round-add" />
            </template>
            {{ $t('page.lowcode.apiConfig.versionManagement.createVersion') }}
          </NButton>
          <NButton @click="refreshVersions" :loading="versionsLoading">
            <template #icon>
      ...
      <NSpace vertical :size="16">
        <!-- API选择 -->
        <div>
          <h3 class="text-lg font-semibold mb-3">{{ $t('page.lowcode.apiConfig.versionManagement.selectApi') }}</h3>
          <NSpace>
            <NSelect
        ...
        <div v-if="currentApiConfig">
          <h3 class="text-lg font-semibold mb-3">{{ $t('page.lowcode.apiConfig.versionManagement.currentVersion') }}</h3>
          <NCard size="small">
        ...
        <div v-if="selectedApiId">
          <h3 class="text-lg font-semibold mb-3">{{ $t('page.lowcode.apiConfig.versionManagement.versionHistory') }}</h3>
          <NCard size="small">
            <NDataTable
        ...
        <div v-if="compareVersions.length === 2">
          <h3 class="text-lg font-semibold mb-3">{{ $t('page.lowcode.apiConfig.versionManagement.versionCompare') }}</h3>
          <NCard size="small">
    ...
    <!-- 创建版本对话框 -->
    <NModal v-model:show="createVersionVisible" preset="dialog" :title="$t('page.lowcode.apiConfig.versionManagement.createVersion')">
      <NSpace vertical :size="12">
        <NFormItem :label="$t('page.lowcode.apiConfig.versionManagement.versionNumber')">
          <NInput v-model:value="newVersionForm.version" :placeholder="$t('page.lowcode.apiConfig.versionManagement.versionPlaceholder')" />
        </NFormItem>
      ...
      <template #action>
        <NSpace>
          ...
          <NButton type="primary" @click="handleCreateVersion" :loading="createVersionLoading">
            {{ $t('common.confirm') }}
          </NButton>
        </NSpace>
      </template>
...
import {
  ...
  NDataTable, NScrollbar, NModal, NFormItem, NInput
} from 'naive-ui';
...
async function loadVersions(apiId: string) {
  ...
  try {
    ...
    const response = await fetchGetApiConfigVersions(
      selectedProjectId.value,
      currentApiConfig.value.code
    );
...
async function handleRollbackVersion(version: any) {
  ...
  try {
    ...
    window.$message?.success($t('page.lowcode.apiConfig.versionManagement.rollbackSuccess', { version: version.version }));
