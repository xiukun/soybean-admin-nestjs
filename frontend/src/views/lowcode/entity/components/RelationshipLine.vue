...
import type { EntityRelationship, Entity, ConnectionPoint, Point } from '../types';
...
function calculateConnectionPoints(entity: Entity): ConnectionPoint[] {
  ...
  for (let i = 0; i < pointsPerSide; i++) {
    ...
    points.push({
      ...
      position: 'top',
      offset,
      point: { x: Math.round(x + width * offset), y: y }
    });
    ...
    points.push({
      ...
      position: 'left',
      offset,
      point: { x: x, y: Math.round(y + height * offset) }
    });
  }
...
function getLinePath(relationship: EntityRelationship): string {
  ...
  // 计算最优连接点
  const { source: sourcePoint, target: targetPoint } = findOptimalConnectionPoints(sourceEntity, targetEntity, relationship);
...
function findOptimalConnectionPoints(
  ...
  const sameEntityRelationships = props.relationships.filter(rel => 
  ...
  const relationshipIndex = currentRelationship ? 
    sameEntityRelationships.findIndex(rel => rel.id === currentRelationship.id) : 0;
 ...
 * @returns SVG 路径字符串
 */
function calculateSmartPath(sourcePoint: Point, targetPoint: Point, sourceEntity: Entity, targetEntity: Entity): string {
