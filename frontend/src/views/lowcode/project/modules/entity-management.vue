...
const columns: DataTableColumns<Entity> = [
  ...
  { title: $t('page.lowcode.entity.category'), key: 'category', width: 100 },
  {
    ...
    width: 100,
    render: row =>
      h('NTag', { type: getStatusType(row.status) }, $t(`page.lowcode.entity.status.${row.status.toLowerCase()}`))
  },
...
function handleDesignFields(entity: Entity) {
  // 跳转到字段设计页面
  window.$message?.info(`设计 ${entity.name} 的字段`);
}
  ...
  <div class="entity-management">
    ...
    <div class="mb-4">
      <NSpace justify="space-between" align="center">
        ...
        <NSpace>
          <NButton type="primary" @click="handleCreateEntity">
            <template #icon>
              <NIcon><icon-mdi-plus /></NIcon>
            </template>
            {{ $t('page.lowcode.entity.create') }}
          </NButton>
          <NButton @click="handleImportEntities">
            <template #icon>
