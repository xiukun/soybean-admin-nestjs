<template>
  <div class="api-management">
    ...
    <NModal v-model:show="showTestModal" preset="card" style="width: 800px">
      <template #header>
        {{ $t('page.lowcode.api.test') }} - {{ testingApi?.name }}
      </template>
...
const columns: DataTableColumns<Api> = [
  ...
  { title: $t('page.lowcode.api.entity'), key: 'entityName', width: 120 },
  {
    ...
    key: 'authRequired',
    width: 100,
    render: (row) => h('NTag', { type: row.authRequired ? 'warning' : 'success', size: 'small' }, 
  ...
  },
  {
    ...
    render: (row) => h('NTag', { type: getStatusType(row.status) }, 
      $t(`page.lowcode.api.status.${row.status.toLowerCase()}`)
    )
  },
