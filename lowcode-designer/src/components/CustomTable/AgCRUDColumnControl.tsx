...
import { FormItem, Button, Icon, toast, Tag, Spinner, autobind, registerFormItem } from 'amis'
import { TooltipWrapper } from 'amis-ui'
import { JSONPipeIn } from 'amis-editor-core'
import AddColumnModal from './AgAddColumnModal'

import type { FormControlProps } from 'amis'
import type { SortableEvent } from 'sortablejs'
import type { ColumnSchema } from 'amis/lib/renderers/Table2/index'
import { DSBuilderInterface } from 'amis-editor'
...
> {
  ...
  initDragging() {
    ...
    this.sortable = new Sortable(
      dom.querySelector(`.${cx('ae-CRUDConfigControl-list')}`) as HTMLUListElement,
      {
        ...
        onEnd: (e: SortableEvent) => {
          ...
          if (e.oldIndex < parent.childNodes.length - 1) {
            parent.insertBefore(
              e.item,
              parent.childNodes[e.oldIndex > e.newIndex ? e.oldIndex + 1 : e.oldIndex],
            )
          } else {
  ...
  handleAddModalConfirm = (scaffold: DesignColumnSchema) => {
    ...
    options.push({
      label: typeof scaffold.label === 'string' ? scaffold.label : scaffold.name,
      value: scaffold.name,
      nodeId: scaffold.$$id,
  ...
  handleDelete = async (item: Option, index: number) => {
  ...
  renderOption = (item: Option, index: number) => {
    const { classnames: cx, data: ctx, render, popOverContainer, env } = this.props

    return (
      <li key={index} className={cx('ae-CRUDConfigControl-list-item', 'is-draggable')}>
        <TooltipWrapper
          tooltip={{
            content: item.label,
            tooltipTheme: 'dark',
  ...
  render() {
    ...
    return (
      <div className={cx('ae-CRUDConfigControl')}>
        {loading ? (
          <Spinner show tip="字段加载中" tipPlacement="bottom" size="sm" className={cx('flex')} />
        ) : (
          <>
            {this.renderHeader()}
...
export class AgCRUDColumnControlRenderer extends CRUDColumnControl {}
