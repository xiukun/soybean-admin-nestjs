...
import type { CRUDColumnControlState } from './AgCRUDColumnControl'
import { DSBuilderInterface, DSFeatureEnum, DSFeatureType, ModelDSBuilderKey } from 'amis-editor'
      ...
      try {
        schema = await form.submit?.(async (values: FormData) => {
          ...
          if (values.colType === 'field') {
            ...
            const fields = (ctx?.columns ?? []).map((item: any) => ({
          ...
          scaffold.label = scaffold.title
          return Promise.resolve(JSONPipeIn(omit(scaffold, ['key', 'title'])))
        })
      } catch (error: any) {
