...
import { FormControlProps, isObject, isValidApi, normalizeApi } from 'amis-core'
import { reaction, type IReactionDisposer } from 'mobx'
import { Button, InputTableColumnProps, toast } from 'amis-ui'
import { DSFeatureEnum, DSFeatureType, ScaffoldField } from 'amis-editor'
...
export class FieldSetting extends React.Component<FieldSettingProps, FieldSettingState> {
  static defaultProps = {
    config: {
      showInputType: true,
  ...
  handleGenerateFields = async (e: React.MouseEvent<any>) => {
    const { store, renderer, feat, env, manager, data: ctx, onAutoGenerateFields } = this.props
    const scaffoldData = store?.data
    let api =
      renderer === 'form' ? scaffoldData?.initApi : renderer === 'crud' ? scaffoldData?.listApi : ''

    if (
      !api ||
      (renderer === 'form' && feat !== DSFeatureEnum.Edit && feat !== DSFeatureEnum.View)
    ) {
      return
    }
    ...
    if (onAutoGenerateFields && typeof onAutoGenerateFields === 'function') {
      try {
        ...
        toast.warning(error.message ?? 'API返回格式不正确，请查看接口响应格式要求')
      }
    } else {
      ...
      try {
        ...
        if (!result.ok) {
          toast.warning(
            result.defaultMsg ?? result.msg ?? 'API返回格式不正确，请查看接口响应格式要求',
          )
  ...
  renderFooter = () => {
    ...
    const showAutoGenBtn =
      (isForm && (feat === DSFeatureEnum.Edit || feat === DSFeatureEnum.View)) ||
    ...
    return showAutoGenBtn ? (
      ...
      >
        <Button
          ...
          disabledTip={{
            content: loading ? '数据处理中...' : isForm ? '请先填写初始化接口' : '请先填写接口',
            tooltipTheme: 'dark',
          }}
  ...
  render() {
    const { render, classnames: cx, name = 'items', renderer, config, feat } = this.props
    const { showDisplayType, showInputType } = config || {}
    const isFirstStep = this.isFirstStep()
    const fields = this.state.fields.concat()
    const fieldsMap = {
      fieldName: getI18N(i18nExtendEnum.字段名称),
      fieldTitle: getI18N(i18nExtendEnum.字段标题),
      title: getI18N(i18nExtendEnum.标题),
    }
    return (
      <>
        {this.renderFooter()}
            ...
            columns: [
              {
                type: 'input-text',
                name: 'name',
                label: fieldsMap.fieldName, //'字段名称',
                placeholder: fieldsMap.fieldName, //'字段名称',
              },
              {
                type: 'input-text',
                name: 'label',
                label: fieldsMap.title, //'标题',
                placeholder: fieldsMap.fieldTitle, //'字段标题',
              },
              showInputType && !(renderer === 'crud' && feat === 'List' && !isFirstStep)
                ? {
                    type: 'select',
                    name: 'inputType',
                    label: getI18N(i18nExtendEnum.输入类型), //'输入类型',
                    options: [
                      {
                        label: '单行文本框',
                        value: 'input-text',
              ...
              showDisplayType
                ? {
                    ...
                    options: [
                      ...
                      { label: '隐藏', value: 'hidden', typeKey: 'value' },
                    ],
                  }
                : undefined,
            ].filter(Boolean),
...
export default class AeFieldSettingRefactor extends FieldSetting {}
